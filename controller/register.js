...
const RegisterUser=async(req,res)=>{
    try {
       ...
       console.log("Name",Name,"email",email,"password",password,"profilePic",profilePic,"role",role);
...
const Login=async(req,res)=>{
   try {
       ...
       const token=jwt.sign({id:findUser._id},"Anand123@",{expiresIn:"1d",})
       res.cookie("token",token);
...
const UpdateUserApi = async (req, res) => {
  try {
    ...
    if (!Name || Name.trim() === '') {
      return res.status(400).json({ msg: 'Name is required and cannot be empty.' });
    }
    if (!profilePic || profilePic.trim() === '') {
      return res.status(400).json({ msg: 'Profile picture is required and cannot be empty.' });
    }
...
module.exports={RegisterUser,Login,getProfile,AllUser,UpdateUserApi}
